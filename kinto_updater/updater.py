...
class Updater(object):
    ...
    def __init__(self, bucket, collection, server_url=None,
                 ...
                 signer_instance=None, settings=None):
        self.client = Client(
    ...
    def check_data_validity(self, records, signature):
        local_hash = hasher.compute_hash(records.values())
        self.signer_instance.verify(local_hash, signature)
    ...
    def add_records(self, new_records):
        ...
        if records:
            if 'signature' not in collection_data:
                msg = "Unable to verify unsigned data"
                raise exceptions.UpdaterException(msg)
        ...
        with self.client.batch() as batch:
            ...
            signature = self.signer_instance.sign(new_hash)
