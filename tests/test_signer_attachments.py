...
class SignerAttachmentsTest(BaseWebTest, unittest.TestCase):
    ...
    def initialize(self):
        r = self.app.post_json(self.source_collection + "/records",
                               {"data": {"title": "hello"}},
                               headers=self.headers)
        r = r.json["data"]
        uri = self.source_collection + "/records/" + r["id"] + "/attachment"
        self.upload_file(uri=uri, files=[('attachment', 'image.jpg', b'--fake--')],
                         headers=self.headers)
    ...
    def test_attachment_is_published_on_final_collection(self):
        ...
        r = self.app.get(self.destination_collection + "/records",
                         headers=self.headers)
    ...
    def test_attachment_can_be_replaced(self):
        ...
        self.upload_file(uri=uri, files=[('attachment', 'image.jpg', b'--other-fake--')],
                         headers=self.headers)
